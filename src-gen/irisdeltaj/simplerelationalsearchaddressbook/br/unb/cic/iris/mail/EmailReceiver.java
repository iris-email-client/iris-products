...
public class EmailReceiver implements StoreListener, FolderListener {
	...
	public List<EmailMessage> getMessages(String folderName, SearchTerm
		searchTerm) throws EmailException {
	...
	public List<EmailMessage> getMessages(String folderName, int begin, int end)
	...
	public List<EmailMessage> getMessages(String folderName, int seqnum) throws
	EmailException {
		List<EmailMessage> messages = new ArrayList<EmailMessage>();
		Folder folder = openFolder(folderName);
		try {
	...
	private List<EmailMessage> convertToIrisMessage(Message [] messagesRetrieved)
	throws EmailException {
		List<EmailMessage> messages = new ArrayList<EmailMessage>();
		int cont = 0;
	...
	private Folder openFolder(String folderName, int openType) throws
	EmailException {
		try {
	...
	private String getText(Message message) throws IOException, MessagingException
	{
		...
		if(message instanceof MimeMessage) {
			...
			if(contentObject instanceof Multipart) {
				...
				System.out.printf("WARNING: not a mime part or multipart %s",
					message.toString());
				result = null;
			}
		}
