...
import
irisdeltaj.simplerelationalsearchaddressbook.br.unb.cic.iris.util.StringUtil;
import
irisdeltaj.simplerelationalsearchaddressbook.br.unb.cic.iris.core.exception.EmailException;
import
irisdeltaj.simplerelationalsearchaddressbook.br.unb.cic.iris.core.exception.EmailMessageValidationException;
import
...
public class EmailSender implements TransportListener {
	...
	public static List<String> validateEmailMessage(EmailMessage message) {
		...
		if(message == null) {
			...
			errorMessages.add(MessageBundle.message("error.required.field",
					MessageBundle.message("command.send.label.from")));
		}
	...
	private Transport createTransport() throws MessagingException {
		...
		Transport transport =
		session.getSession().getTransport(provider.getTransportProtocol());
		transport.addTransportListener(this);
		transport.addConnectionListener(session);
		return transport;
	}
	private Message createMessage(final EmailMessage email) throws
