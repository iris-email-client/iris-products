...
public final class EmailDAO extends AbstractDAO<EmailMessage> implements IEmailDAO {
	...
	public List<EmailMessage> search(String queryStr) throws EmailException {
		String FIND = "select e FROM EmailMessage e where e.from like :text or e.to like :text or e.cc like :text or e. cc like :text or e.subject like :text or e.message like :text";
		List<EmailMessage> result = new ArrayList<EmailMessage>();
		try {
			startSession(false);
			result = session.createQuery(FIND).setParameter("text", "%" + queryStr + "%").list();
		} catch (Exception e) {
			e.printStackTrace();
